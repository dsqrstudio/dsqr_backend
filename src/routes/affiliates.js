...
    
    // FIX 1: Map 'url' to 'src' for frontend compatibility
...
    res.json({ ok: true, items: formattedItems });
  } catch (err) {
    console.error(err);
...
    if (!Array.isArray(payload)) return res.status(400).json({ ok: false, error: 'Invalid payload' });

    // Validate every URL server-side (using the incoming 'src' which maps to 'url')
    for (const p of payload) {
      if (!p.src || !isValidHttpUrl(p.src)) { // Using p.src for validation
        return res.status(400).json({ ok: false, error: `Invalid URL: ${p.src}` });
      }
    }
...
      if (p.id) { // Use p.id from frontend as MongoDB's _id
        // update existing
        const doc = await AffiliateImage.findByIdAndUpdate(p.id, docData, { new: true });
...
    await AffiliateImage.deleteMany({ _id: { $nin: incomingIds } });

    // Fetch and format final list
...
    const updated = await AffiliateImage.findByIdAndUpdate(id, { alt, order, active }, { new: true });
